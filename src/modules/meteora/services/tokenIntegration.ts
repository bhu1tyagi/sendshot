...
/**
 * Creates a token in the database after it has been created on-chain with Meteora
 * This ensures the token is properly recorded and displayed in the UI
 */
export async function registerMeteoraToken(
  tokenData: Omit<TokenCreationData, 'protocolType'>
): Promise<boolean> {
  try {
    console.log('[registerMeteoraToken] Registering token:', {
      name: tokenData.name,
    ...
    if (!tokenData.creatorId || typeof tokenData.creatorId !== 'string') {
      console.error('[registerMeteoraToken] Invalid creatorId:', tokenData.creatorId);
      return false;
    }
    
    // Add protocol type for Meteora
    ...
    console.error('Error registering Meteora token:', error);
    return false;
  }
} 